...
import { CreateProductKeyDto, ProductKeyResponseDto, GetAllProductKeysResponseDto, UpdateProductKeyDto } from './model/product-key.dto';
  ...
  export class ProductKeyService {
    ...
    async getAllProductKeys(
      ...
      const totalItems = await this.prisma.productKey.count({
        where: { key: { contains: searchQuery, mode: 'insensitive' } },
      });
      ...
      const productKeyResponseDtos: ProductKeyResponseDto[] = keys.map(
        (key) => ({
          id: key.id,
          key: key.key,
